...
- 3.3.7 的松弛下手太重（一天十几次互动把「想她」砍到 0.1 附近、平台连成片不再长），改三处：松弛只作用于 静息线×0.35 以上的部分（「想她」底线约 0.29，再多亲昵也砍不穿）；一小时内同类互动效果减半；饱足平台不叠加，上限 2.5 小时，时长系数 relief×6。
- 服务端替判互动类型的节流可配：`INTERACTION_CLASSIFY_MIN_MINUTES`（默认 8 分钟，原来写死）。

## 3.3.7 — 2026-09-18

### 去饱和·第一步

- 关系类互动真的能把驱力拉下来：想她 / 惦记 / 馋 / 性欲 / 陪伴 的 relief 加大，单次上限 0.35→0.60；relief ≥0.15 的维进饱足平台，平台期内不长。
- 状态新增 `driveTrail`：结算时每 30 分钟记一个全维快照，留 48 个点（24 小时）；`driveTrend(state, key, now)` 给两小时变化。
- 「此刻」的驱力措辞统一成 `driveLevel`：静（<0.25）/ 涌（≥0.90 或高出自己的静息线 0.05）/ 落（两小时掉 ≥0.08）/ 涨（两小时起 ≥0.08）/ 平。之前静息线本身就高过旧的"涌"线，他一睁眼永远是"想她（涌）"，字没有信息量。
