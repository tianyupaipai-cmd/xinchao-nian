...
- 念头池回推驱力改两处：同一个念头只回推一次（原来最多 3 次 ×0.18）；回推只把驱力顶到 0.85 为止，已经 ≥0.85 不再推。原因：召回按最强驱力偏置 → 浮现的闪念都挂在同一维 → 念头再回推同一维，闭环把「想沉淀」钉死在 1.00（9/7 起 17 次回推全在 reflection）；而沉淀维只有真的记一笔 reflection 才会降。念头的寿命（3 次结算）不变。
